...
const std::vector<InstField> vop_inst_fields = {
	{ 31, 27, 0x0A, true, RegClass::NotReg, "funct5" },
	{ 26, 26, 0x00, false, RegClass::NotReg, "imm5" },
	{ 25, 25, 0x00, false, RegClass::NotReg, "vm" },
	{ 24, 20, 0x00, false, RegClass::Vector, "vs2" },
	{ 19, 15, 0x00, false, RegClass::NotReg, "imm4_0" },
...
template <typename Ts2, typename Td> int run_self_result(c_data &cur_data)
{
	...
	uint32_t imm = (cur_data.map_reg_index["imm5"] << 5) |
		       cur_data.map_reg_index["imm4_0"];
	...
	for (int j = 0; j < vector_cfg.len; j++) {
		...
		selfcheck_data[j] = ror<Ts2>(vs2_data[j], imm);
	}
	return 0;
}
