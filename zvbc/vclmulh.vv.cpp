...
const std::vector<InstField> vop_inst_fields = {
	{ 31, 26, 0x0d, true, RegClass::NotReg, "funct6" },
...
// Carry-less multiplication: high 64 bits of 128-bit product in GF(2)
uint64_t clmulh(uint64_t a, uint64_t b)
{
	uint64_t result = 0;
	for (int i = 1; i <= 63; i++) {
		if ((b >> i) & 1)
...
int run_self_result(c_data &cur_data)
{
	...
	for (int j = 0; j < vector_cfg.len; j++) {
		...
		// Perform carry-less multiplication high operation
		selfcheck_data[j] = static_cast<Td>(
			clmulh(static_cast<uint64_t>(vs1_data[j]),
...
int main(int argc, char *argv[])
{
	...
	for (it = 0; it < run_times; it++) {
		...
		int has_error = 0;
		// vclmulh is only defined for SEW=64, all other SEW values are illegal
