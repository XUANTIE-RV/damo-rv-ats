...
const std::vector<InstField> vop_inst_fields = {
	{ 31, 26, 0x0c, true, RegClass::NotReg, "funct6" },
...
// Function to implement carryless multiplication operation for 64-bit values
...
int run_self_result(c_data &cur_data)
{
	...
	for (int j = 0; j < vector_cfg.len; j++) {
		...
		// Perform carryless multiplication: rs1 * vs2[j]
		selfcheck_data[j] = static_cast<Td>(
			clmul(static_cast<uint64_t>(*rs1_data),
...
int main(int argc, char *argv[])
{
	...
	for (it = 0; it < run_times; it++) {
		...
		int has_error = 0;
		// vclmul is only defined for SEW=64, all other SEW values are illegal
