...
const std::vector<InstField> vop_inst_fields = {
	{ 31, 26, 0x3A, true, RegClass::NotReg, "funct6" },
...
int check_illegal(c_data &cur_data)
{
	...
	int illegal = !(VectorRegValidator::validate(
			      VregOperand::two_pow(vd),
			      {
				      VregOperand::one_pow(vs2),
			      },
			      vm_bit, Presets::WIDEN)) ||
		      (vd == vs2);
	print_illegal_status(illegal);
	return illegal;
}

// Ts2 = signed SEW (vs2), Td = signed 2*SEW
// rs1 is treated as unsigned SEW
template <typename Ts2, typename Td> int run_self_result(c_data &cur_data)
{
	...
	typedef std::make_unsigned_t<Ts2> Us2;
	...
	for (int j = 0; j < vector_cfg.len; j++) {
		...
		// vs2 is signed, rs1 is unsigned (truncated to SEW bits, then zero-extended)
		selfcheck_data[j] =
			static_cast<Td>(vs2_data[j]) *
			static_cast<Td>(static_cast<Us2>(*rs1_data));
	}
	return 0;
}
...
int per_run(int it, c_cfg &cur_cfg, c_data &cur_data)
{
	...
	load_multi_vector<Td, Ts2, uint8_t>(
		...
		{ lmul_pow(vector_cfg.lmul, 1), vector_cfg.lmul, lmul_m1 },
...
int main(int argc, char *argv[])
{
	...
	for (it = 0; it < run_times; it++) {
		...
		// Ts2 = signed SEW (vs2), Td = signed 2*SEW, rs1 unsigned handled in run_self_result
		case sew_e8:
			has_error =
				per_run<int8_t, int16_t>(it, cur_cfg, cur_data);
			break;
		case sew_e16:
			has_error = per_run<int16_t, int32_t>(it, cur_cfg,
							      cur_data);
			break;
		case sew_e32:
			has_error = per_run<int32_t, int64_t>(it, cur_cfg,
							      cur_data);
			break;
		default:
			break;
