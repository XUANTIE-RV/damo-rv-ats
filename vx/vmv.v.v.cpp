...
template <typename Ts1, typename Td> int run_self_result(c_data &cur_data)
{
	int vs1 = cur_data.map_reg_index["vs1"];
	int vd = cur_data.map_reg_index["vd"];
...
template <typename Ts1, typename Td>
int per_run(int it, c_cfg &cur_cfg, c_data &cur_data)
{
	...
	store_multi_preinst_vector<Td, Ts1>(insts, cur_data, { "vd", "vs1" },
					    { vector_cfg.lmul,
					      vector_cfg.lmul },
					    { vector_cfg.len, vector_cfg.len });
	...
	save_multi_preinst_value_to_common<Td, Ts1>(
		cur_data, { "vd", "vs1" }, { vector_cfg.len, vector_cfg.len });

	run_self_result<Ts1, Td>(cur_data);
