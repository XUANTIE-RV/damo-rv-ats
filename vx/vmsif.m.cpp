...
const std::vector<InstField> vop_inst_fields = {
	...
	{ 19, 15, 0x03, true, RegClass::NotReg,
	  "vs1" }, // vs1 fixed to 0x03 for vmsif.m
...
int run_self_result(c_data &cur_data)
{
	...
	// Write 1 to all active elements up to and including the first active element that is 1,
	// then write 0 to all following active elements.
	...
	for (int j = 0; j < vector_cfg.len; j++) {
		...
		if (!found_first) {
			// Before or at the first set bit: write 1
			selfcheck_data[byte_idx] |= (1u << bit_idx);

			if (vs2_bit) {
				// Found the first set bit: mark it, but current element still gets 1
				found_first = true;
			}
		} else {
