...
const std::vector<InstField> vop_inst_fields = {
	{ 31, 26, 0x30, true, RegClass::NotReg, "funct6" },
...
// =============================================================================
// vwredsumu.vs: 2*SEW = 2*SEW + sum(zero-extend(SEW))
// Unsigned widening sum reduction.
// Zero-extends SEW-wide vector elements to 2*SEW before summing,
...
template <typename Ts2, typename Td> int run_self_result(c_data &cur_data)
{
	...
	for (int j = 0; j < vector_cfg.len; j++) {
		...
		// Zero-extend SEW-wide element to 2*SEW before adding
		typedef typename std::make_unsigned<Ts2>::type UTs2;
		accumulator = accumulator +
			      static_cast<Td>(static_cast<UTs2>(vs2_data[j]));
	}
