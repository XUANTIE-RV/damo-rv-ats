...
int check_illegal(c_data &cur_data)
{
	...
	int illegal = !(VectorRegValidator::validate(VregOperand::one_pow(vd),
						     {}, vm_bit, config));
...
template <typename Td> int run_self_result(c_data &cur_data)
{
	...
	for (int j = 0; j < vector_cfg.len; j++) {
		...
		// vmv.v.i: vd[i] = sign_extend(imm, 5) (splat)
		selfcheck_data[j] = static_cast<Td>(imm);
	}
	return 0;
}
...
template <typename Td> int per_run(int it, c_cfg &cur_cfg, c_data &cur_data)
{
	...
	save_multi_preinst_value_to_common<Td>(cur_data, { "vd" },
					       { vector_cfg.len });
