...
// Ts1 = unsigned SEW (vs1), Ts2 = signed SEW (vs2), Td = signed 2*SEW
...
int main(int argc, char *argv[])
{
	...
	for (it = 0; it < run_times; it++) {
		...
		// Ts1 = unsigned (vs1), Ts2 = signed (vs2), Td = signed 2*SEW
		case sew_e8:
			has_error = per_run<uint8_t, int8_t, int16_t>(
				it, cur_cfg, cur_data);
			break;
		case sew_e16:
			has_error = per_run<uint16_t, int16_t, int32_t>(
				it, cur_cfg, cur_data);
			break;
		case sew_e32:
			has_error = per_run<uint32_t, int32_t, int64_t>(
				it, cur_cfg, cur_data);
			break;
		default:
			break;
