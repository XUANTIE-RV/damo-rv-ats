...
const std::vector<InstField> vop_inst_fields = {
	{ 31, 26, 0x1F, true, RegClass::NotReg, "funct6" },
...
template <typename Ts2> int run_self_result(c_data &cur_data)
{
	...
	Ts2 rs1_val = static_cast<Ts2>(*rs1_data);
	...
	for (int j = 0; j < vector_cfg.len; j++) {
		...
		if (!vm_bit && !((vm_data[j / 8] >> (j % 8)) & 1)) {
			continue;
		}

		int result = (vs2_data[j] > rs1_val) ? 1 : 0;
...
template <typename Ts2> int per_run(int it, c_cfg &cur_cfg, c_data &cur_data)
{
	...
	load_multi_vector<uint8_t, Ts2, uint8_t>(
		insts, cur_data, { "vd", "vs2", "vm" },
		{ lmul_m1, vector_cfg.lmul, lmul_m1 },
	...
	store_multi_preinst_vector<uint8_t, Ts2, uint8_t>(
		insts, cur_data, { "vd", "vs2", "vm" },
		{ lmul_m1, vector_cfg.lmul, lmul_m1 },
		{ vlen, vector_cfg.len, vlen });
	...
	save_multi_preinst_value_to_common<uint64_t, uint8_t, Ts2, uint8_t>(
		cur_data, { "rs1", "vd", "vs2", "vm" },
		{ 1, vlen, vector_cfg.len, vlen });
...
int main(int argc, char *argv[])
{
	...
	for (it = 0; it < run_times; it++) {
		...
		case sew_e8:
			has_error = per_run<int8_t>(it, cur_cfg, cur_data);
			break;
		case sew_e16:
			has_error = per_run<int16_t>(it, cur_cfg, cur_data);
			break;
		case sew_e32:
			has_error = per_run<int32_t>(it, cur_cfg, cur_data);
			break;
		case sew_e64:
			has_error = per_run<int64_t>(it, cur_cfg, cur_data);
			break;
		default:
			break;
		}
