...
const std::vector<InstField> vop_inst_fields = {
	...
	{ 19, 15, 0x10, true, RegClass::NotReg,
	  "vs1" }, // vs1 fixed to 16 (0x10)
...
template <typename Ts2, typename Td> int run_self_result(c_data &cur_data)
{
	...
	if (vm_bit) {
		...
		for (int j = 0; j < vector_cfg.len; j++) {
			int row_vs2 =
				j /
				8; // Changed from SEW to 8 since we're accessing bytes
			int col_vs2 = j % 8;
			uint8_t vs2_bit = (reinterpret_cast<uint8_t *>(
						   vs2_data)[row_vs2] >>
					   col_vs2) &
					  1;

			selfcheck_data[j] = sum;
	...
	for (int j = 0; j < vector_cfg.len; j++) {
		int row = j / 8;
		int col = j % 8;
		uint8_t vm_bit_curr = (vm_data[row] >> col) & 1;
...
int main(int argc, char *argv[])
{
	...
	for (it = 0; it < run_times; it++) {
		...
		case sew_e16:
			...
			break;
		case sew_e32:
			has_error = per_run<uint8_t, uint32_t>(it, cur_cfg,
							       cur_data);
			break;
		case sew_e64:
			has_error = per_run<uint8_t, uint64_t>(it, cur_cfg,
							       cur_data);
			break;
		default:
			break;
