...
template <typename Ts2, typename Td> int run_self_result(c_data &cur_data)
{
	...
	uint8_t *v0_data = static_cast<uint8_t *>(
	...
	for (int j = 0; j < vector_cfg.len; j++) {
		...
		// vmerge operates on all body elements
		// vd[i] = v0.mask[i] ? imm : vs2[i]
		int mask_bit = (v0_data[j / 8] >> (j % 8)) & 1;
		if (mask_bit) {
...
int main(int argc, char *argv[])
{
	...
	for (it = 0; it < run_times; it++) {
		...
		case sew_e8:
			has_error =
				per_run<int8_t, int8_t>(it, cur_cfg, cur_data);
			break;
		case sew_e16:
			has_error = per_run<int16_t, int16_t>(it, cur_cfg,
							      cur_data);
			break;
		case sew_e32:
			has_error = per_run<int32_t, int32_t>(it, cur_cfg,
							      cur_data);
			break;
		case sew_e64:
			has_error = per_run<int64_t, int64_t>(it, cur_cfg,
							      cur_data);
			break;
		default:
			break;
