...
template <typename Ts2, typename Td> int run_self_result(c_data &cur_data)
{
	...
	for (int j = 0; j < vector_cfg.len; j++) {
		...
		selfcheck_data[j] = static_cast<Td>(
			static_cast<Ts2>(vs2_data[j]) >> static_cast<Ts2>(imm));
	}
...
int main(int argc, char *argv[])
{
	...
	for (it = 0; it < run_times; it++) {
		...
		case sew_e8:
			has_error = per_run<uint16_t, uint8_t>(it, cur_cfg,
							       cur_data);
			break;
		case sew_e16:
			...
			break;
		case sew_e32:
			has_error = per_run<uint64_t, uint32_t>(it, cur_cfg,
								cur_data);
			break;
		default:
			break;
