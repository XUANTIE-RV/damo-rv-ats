...
const std::vector<InstField> vop_inst_fields = {
	{ 31, 26, 0x35, true, RegClass::NotReg, "funct6" },
...
int main(int argc, char *argv[])
{
	...
	for (it = 0; it < run_times; it++) {
		...
		case sew_e8:
			has_error = per_run<int8_t, int16_t, int16_t>(
				it, cur_cfg, cur_data);
			break;
		case sew_e16:
			has_error = per_run<int16_t, int32_t, int32_t>(
				it, cur_cfg, cur_data);
			break;
		case sew_e32:
			has_error = per_run<int32_t, int64_t, int64_t>(
				it, cur_cfg, cur_data);
			break;
		default:
			break;
