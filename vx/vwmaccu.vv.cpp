...
int run_self_result(c_data &cur_data)
{
	...
	for (int j = 0; j < vector_cfg.len; j++) {
		...
		// vd[i] = (unsigned(vs1[i]) * unsigned(vs2[i])) + vd[i]
...
int main(int argc, char *argv[])
{
	...
	for (it = 0; it < run_times; it++) {
		...
		case sew_e8:
			has_error = per_run<uint8_t, uint8_t, uint16_t>(
				it, cur_cfg, cur_data);
			break;
		case sew_e16:
			has_error = per_run<uint16_t, uint16_t, uint32_t>(
				it, cur_cfg, cur_data);
			break;
		case sew_e32:
			has_error = per_run<uint32_t, uint32_t, uint64_t>(
				it, cur_cfg, cur_data);
			break;
		default:
			break;
