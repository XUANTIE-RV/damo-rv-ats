...
const std::vector<InstField> vop_inst_fields = {
	{ 31, 26, 0x0F, true, RegClass::NotReg, "funct6" },
...
template <typename Ts2, typename Td> int run_self_result(c_data &cur_data)
{
	...
	// The immediate is zero-extended (unsigned) for vslidedown.vi
	uint64_t OFFSET = cur_data.map_reg_index["imm"] & 0x1F;
	...
	for (int j = 0; j < vector_cfg.len; j++) {
		...
		// vslidedown behavior:
		// src[i] = (i+OFFSET < VLMAX) ? vs2[i+OFFSET] : 0
		// vd[i] = src[i] if mask enabled
		uint64_t src_idx = (uint64_t)j + OFFSET;
		if (src_idx < VLMAX) {
			selfcheck_data[j] = static_cast<Td>(vs2_data[src_idx]);
		} else {
			selfcheck_data[j] = 0;
		}
	}
