...
const std::vector<InstField> vop_inst_fields = {
	...
	{ 19, 15, 0x07, true, RegClass::NotReg, "simm" },
...
// =============================================================================
// vmv8r.v: Copy 8 vector registers. NREG=8, simm=0x07.
// vd and vs2 must be 8-aligned.
// EEW=SEW, EMUL=8, evl=8*VLEN/SEW.
// =============================================================================
int check_illegal(c_data &cur_data)
{
	int vs2 = cur_data.map_reg_index["vs2"];
	int vd = cur_data.map_reg_index["vd"];
	// NREG=8: vd and vs2 must be 8-aligned
	int illegal = !((vd % 8 == 0) && (vs2 % 8 == 0));
...
template <typename Td> int run_self_result(c_data &cur_data)
{
	...
	// Copy 8 registers: evl = 8*VLEN/SEW
	int evl = 8 * vlen / sizeof(Td);
	for (int j = 0; j < evl; j++) {
...
template <typename Td> int per_run(int it, c_cfg &cur_cfg, c_data &cur_data)
{
	...
	store_multi_afterinst_vector<Td>(insts, cur_data, { "vd" }, { lmul_m8 },
					 { (uint64_t)evl });
