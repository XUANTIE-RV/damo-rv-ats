...
const std::vector<InstField> vop_inst_fields = {
	...
	{ 19, 15, 0x01, true, RegClass::NotReg, "simm" },
...
// =============================================================================
// vmv2r.v: Copy 2 vector registers. NREG=2, simm=0x01.
// vd and vs2 must be 2-aligned.
// EEW=SEW, EMUL=2, evl=2*VLEN/SEW.
// =============================================================================
int check_illegal(c_data &cur_data)
{
	int vs2 = cur_data.map_reg_index["vs2"];
	int vd = cur_data.map_reg_index["vd"];
	// NREG=2: vd and vs2 must be 2-aligned
	int illegal = !((vd % 2 == 0) && (vs2 % 2 == 0));
...
template <typename Td> int per_run(int it, c_cfg &cur_cfg, c_data &cur_data)
{
	...
	store_multi_afterinst_vector<Td>(insts, cur_data, { "vd" }, { lmul_m2 },
					 { (uint64_t)evl });
