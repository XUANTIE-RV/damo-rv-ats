...
const std::vector<InstField> vop_inst_fields = {
	{ 31, 26, 0x3F, true, RegClass::NotReg, "funct6" },
...
// Ts1 = signed SEW (vs1), Ts2 = unsigned SEW (vs2), Td = signed 2*SEW
...
int run_self_result(c_data &cur_data)
{
	...
	for (int j = 0; j < vector_cfg.len; j++) {
		...
		selfcheck_data[j] = static_cast<Td>(vs1_data[j]) *
...
int per_run(int it, c_cfg &cur_cfg, c_data &cur_data)
{
	...
	load_multi_vector<Td, Ts1, Ts2, uint8_t>(
		...
		{ lmul_pow(vector_cfg.lmul, 1), vector_cfg.lmul,
...
int main(int argc, char *argv[])
{
	...
	for (it = 0; it < run_times; it++) {
		...
		// Ts1 = signed (vs1), Ts2 = unsigned (vs2), Td = signed 2*SEW
		case sew_e8:
			has_error = per_run<int8_t, uint8_t, int16_t>(
				it, cur_cfg, cur_data);
			break;
		case sew_e16:
			has_error = per_run<int16_t, uint16_t, int32_t>(
				it, cur_cfg, cur_data);
			break;
		case sew_e32:
			has_error = per_run<int32_t, uint32_t, int64_t>(
				it, cur_cfg, cur_data);
			break;
		default:
			break;
