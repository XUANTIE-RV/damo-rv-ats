...
const std::vector<InstField> vop_inst_fields = {
	...
	{ 19, 15, 0x00, false, RegClass::NotReg, "imm" },
...
template <typename Ts2> int run_self_result(c_data &cur_data)
{
	...
	Ts2 imm_val = static_cast<Ts2>(imm);
	...
	for (int j = 0; j < vector_cfg.len; j++) {
		...
		int result = (vs2_data[j] <= imm_val) ? 1 : 0;
...
template <typename Ts2> int per_run(int it, c_cfg &cur_cfg, c_data &cur_data)
{
	...
	save_multi_preinst_value_to_common<uint8_t, Ts2, uint8_t>(
		cur_data, { "vd", "vs2", "vm" },
		{ vlen, vector_cfg.len, vlen });
