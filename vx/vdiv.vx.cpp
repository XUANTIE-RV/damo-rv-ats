...
int run_self_result(c_data &cur_data)
{
	...
	for (int j = 0; j < vector_cfg.len; j++) {
		...
		// Signed divide: RISC-V standard behavior
		if (*rs1_data == 0) {
			// Divide by zero: result is -1
			selfcheck_data[j] = static_cast<Td>(-1);
		} else if (vs2_data[j] == std::numeric_limits<Td>::min() &&
			   *rs1_data == static_cast<Ts1>(-1)) {
			// Signed overflow: INT_MIN / -1 = INT_MIN
			selfcheck_data[j] = std::numeric_limits<Td>::min();
		} else {
			selfcheck_data[j] = static_cast<Td>(vs2_data[j]) /
					    static_cast<Td>(*rs1_data);
		}
	}
	return 0;
}
