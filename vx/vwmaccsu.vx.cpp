...
// Ts2 = unsigned SEW (vs2), Td = signed 2*SEW
// rs1 is treated as signed SEW
template <typename Ts2, typename Td> int run_self_result(c_data &cur_data)
{
	...
	typedef std::make_signed_t<Ts2> Ss2;
	...
	for (int j = 0; j < vector_cfg.len; j++) {
		...
		selfcheck_data[j] =
			static_cast<Td>(static_cast<Ss2>(*rs1_data)) *
...
int main(int argc, char *argv[])
{
	...
	for (it = 0; it < run_times; it++) {
		...
		// Ts2 = unsigned (vs2), Td = signed 2*SEW, rs1 signed handled in run_self_result
		case sew_e8:
