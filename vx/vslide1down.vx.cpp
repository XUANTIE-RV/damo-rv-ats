...
template <typename Ts2, typename Td> int run_self_result(c_data &cur_data)
{
	...
	// x[rs1] is XLEN bits; if XLEN > SEW, take low SEW bits; if XLEN < SEW, sign-extend
	...
	for (int j = 0; j < vector_cfg.len; j++) {
		...
		// i == vl-1: vd[vl-1] = x[rs1] (truncated to SEW bits)
		if (j == (int)(vector_cfg.len - 1)) {
			selfcheck_data[j] = static_cast<Td>(*rs1_data);
		} else {
			selfcheck_data[j] = static_cast<Td>(vs2_data[j + 1]);
		}
	}
