...
// =============================================================================
// vmv.s.x: vd[0] = x[rs1]
// Copies the scalar integer register to element 0 of the destination vector.
// If SEW < XLEN, the least-significant bits are copied.
// If SEW > XLEN, the value is sign-extended to SEW bits.
// Other elements (0 < index < VLEN/SEW) are treated as tail elements
// using the current tail agnostic/undisturbed policy.
...
// vm=0 is reserved (illegal). vs2 is fixed to 0x00.
// =============================================================================
int check_illegal(c_data &cur_data)
{
	...
	// vmv.s.x does not require any such checks since:
	//   - vm is always 1 (no mask check)
	//   - LMUL is ignored (no alignment check)
	//   - vs2 is fixed to 0 (no source overlap check)
...
template <typename Td> int run_self_result(c_data &cur_data)
{
	...
	// vmv.s.x: vd[0] = x[rs1]
	// Only executes if vl > 0 and vstart < vl (per SPEC)
	// If vstart >= vl, no operation is performed and vd is not updated
	if (vector_cfg.len > 0 && vector_cfg.vstart < vector_cfg.len) {
		selfcheck_data[0] = static_cast<Td>(*rs1_data);
	}
...
template <typename Td> int per_run(int it, c_cfg &cur_cfg, c_data &cur_data)
{
	...
	save_multi_preinst_value_to_common<Td, uint64_t>(
		cur_data, { "vd", "rs1" }, { vector_cfg.len, 1 });
