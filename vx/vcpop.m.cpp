...
const std::vector<InstField> vop_inst_fields = {
	...
	{ 19, 15, 0x10, true, RegClass::NotReg, "rs1" },
...
int run_self_result(c_data &cur_data)
{
	...
	if (vector_cfg.vstart != 0) {
		selfcheck_data[0] = 0;
		return -1;
	}

	int count = 0;
	int vl = vector_cfg.len;
	...
	for (int i = 0; i < vl; i++) {
		...
		if (vm_bit == 1) {
			flag = (reinterpret_cast<uint8_t *>(
					vs2_data)[byte_idx] >>
				bit_idx) &
			       1;
		}

		else if (vm_bit == 0 && vs2 == 0) {
			flag = (vm_data[byte_idx] >> bit_idx) & 1;
		}
	...
	selfcheck_data[0] = static_cast<Td>(count);
	return 0;
}
...
int main(int argc, char *argv[])
{
	...
	for (it = 0; it < run_times; it++) {
		...
		case sew_e8:
			has_error = per_run<uint8_t, uint8_t, uint64_t>(
				it, cur_cfg, cur_data);
			break;
		case sew_e16:
			has_error = per_run<uint16_t, uint16_t, uint64_t>(
				it, cur_cfg, cur_data);
			break;
		case sew_e32:
