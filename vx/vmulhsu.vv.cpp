...
const std::vector<InstField> vop_inst_fields = {
	{ 31, 26, 0x26, true, RegClass::NotReg, "funct6" },
...
// vmulhsu: Signed(vs2) * Unsigned(vs1), returning high bits
// Ts2 is signed type, Ts1 is unsigned type, Td is signed type
...
int run_self_result(c_data &cur_data)
{
	...
	for (int j = 0; j < vector_cfg.len; j++) {
		...
		// Multiply signed(vs2) * unsigned(vs1) and take high bits
		// Use signed wide type: sign-extend vs2, zero-extend vs1
		if constexpr (sizeof(Ts1) == 8) {
			// SEW=e64: use __int128
			// vs2 is int64_t (signed), vs1 is uint64_t (unsigned)
			__int128 wide_vs2 = static_cast<__int128>(vs2_data[j]);
			__int128 wide_vs1 = static_cast<__int128>(
				static_cast<unsigned __int128>(vs1_data[j]));
			__int128 wide_product = wide_vs2 * wide_vs1;
			selfcheck_data[j] = static_cast<Td>(wide_product >> 64);
		} else if constexpr (sizeof(Ts1) == 4) {
			// vs2 is int32_t, vs1 is uint32_t
			int64_t wide_vs2 = static_cast<int64_t>(vs2_data[j]);
			int64_t wide_vs1 = static_cast<int64_t>(
				static_cast<uint64_t>(vs1_data[j]));
			int64_t wide_product = wide_vs2 * wide_vs1;
			selfcheck_data[j] = static_cast<Td>(wide_product >> 32);
		} else if constexpr (sizeof(Ts1) == 2) {
			// vs2 is int16_t, vs1 is uint16_t
			int32_t wide_vs2 = static_cast<int32_t>(vs2_data[j]);
			int32_t wide_vs1 = static_cast<int32_t>(
				static_cast<uint32_t>(vs1_data[j]));
			int32_t wide_product = wide_vs2 * wide_vs1;
			selfcheck_data[j] = static_cast<Td>(wide_product >> 16);
		} else {
			// vs2 is int8_t, vs1 is uint8_t
			int16_t wide_vs2 = static_cast<int16_t>(vs2_data[j]);
			int16_t wide_vs1 = static_cast<int16_t>(
				static_cast<uint16_t>(vs1_data[j]));
			int16_t wide_product = wide_vs2 * wide_vs1;
			selfcheck_data[j] = static_cast<Td>(wide_product >> 8);
		}
	}
	return 0;
}
...
int main(int argc, char *argv[])
{
	...
	for (it = 0; it < run_times; it++) {
		...
		int has_error = 0;
		// vmulhsu: Ts1=unsigned(vs1), Ts2=signed(vs2), Td=signed
		switch (vector_cfg.sew) {
		case sew_e8:
			has_error = per_run<uint8_t, int8_t, int8_t>(
				it, cur_cfg, cur_data);
			break;
		case sew_e16:
			has_error = per_run<uint16_t, int16_t, int16_t>(
				it, cur_cfg, cur_data);
			break;
		case sew_e32:
			has_error = per_run<uint32_t, int32_t, int32_t>(
				it, cur_cfg, cur_data);
			break;
		case sew_e64:
			has_error = per_run<uint64_t, int64_t, int64_t>(
				it, cur_cfg, cur_data);
			break;
		default:
			break;
