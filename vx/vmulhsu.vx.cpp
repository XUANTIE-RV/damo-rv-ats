...
const std::vector<InstField> vop_inst_fields = {
	...
	{ 14, 12, OPMVX, true, RegClass::NotReg, "funct3" },
...
// vmulhsu: Signed(vs2) * Unsigned(rs1), returning high bits
...
int run_self_result(c_data &cur_data)
{
	...
	for (int j = 0; j < vector_cfg.len; j++) {
		...
		if constexpr (sizeof(Ts1) == 8) {
			...
			__int128 wide_rs1 = static_cast<__int128>(
				static_cast<unsigned __int128>(*rs1_data));
			__int128 wide_product = wide_vs2 * wide_rs1;
			...
			int64_t wide_rs1 = static_cast<int64_t>(
				static_cast<uint64_t>(*rs1_data));
			int64_t wide_product = wide_vs2 * wide_rs1;
			...
			int32_t wide_rs1 = static_cast<int32_t>(
				static_cast<uint32_t>(*rs1_data));
			int32_t wide_product = wide_vs2 * wide_rs1;
			selfcheck_data[j] = static_cast<Td>(wide_product >> 16);
		} else {
			int16_t wide_vs2 = static_cast<int16_t>(vs2_data[j]);
			int16_t wide_rs1 = static_cast<int16_t>(
				static_cast<uint16_t>(*rs1_data));
			int16_t wide_product = wide_vs2 * wide_rs1;
...
int main(int argc, char *argv[])
{
	...
	for (it = 0; it < run_times; it++) {
		...
		int has_error = 0;
		// vmulhsu: Ts1=unsigned(rs1), Ts2=signed(vs2), Td=signed
