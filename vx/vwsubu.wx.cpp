...
const std::vector<InstField> vop_inst_fields = {
	{ 31, 26, 0x36, true, RegClass::NotReg, "funct6" },
...
int check_illegal(c_data &cur_data)
{
	...
	int illegal = !(
		VectorRegValidator::validate(VregOperand::two_pow(vd),
					     {
...
int per_run(int it, c_cfg &cur_cfg, c_data &cur_data)
{
	...
	load_multi_vector<Td, Ts2, uint8_t>(
		...
		{ lmul_pow(vector_cfg.lmul, 1), lmul_pow(vector_cfg.lmul, 1),
		  lmul_m1 },
	...
	save_multi_preinst_value_to_common<Td, Ts2, uint8_t, Ts1>(
...
int main(int argc, char *argv[])
{
	...
	for (it = 0; it < run_times; it++) {
		...
		case sew_e8:
			has_error = per_run<uint8_t, uint16_t, uint16_t>(
				it, cur_cfg, cur_data);
			break;
		case sew_e16:
			has_error = per_run<uint16_t, uint32_t, uint32_t>(
				it, cur_cfg, cur_data);
			break;
		case sew_e32:
			has_error = per_run<uint32_t, uint64_t, uint64_t>(
				it, cur_cfg, cur_data);
			break;
		default:
			break;
