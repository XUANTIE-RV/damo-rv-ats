...
int run_self_result(c_data &cur_data)
{
	...
	// x[rs1] is XLEN bits, treated as unsigned
	...
	// The index value from scalar register (full XLEN bits, treated as unsigned)
	uint64_t index = *rs1_data;
	...
	for (int j = 0; j < vector_cfg.len; j++) {
		...
		if (index >= VLMAX) {
			...
			selfcheck_data[j] = static_cast<Td>(vs2_data[index]);
		}
	}
	return 0;
}
