...
const std::vector<InstField> vop_inst_fields = {
	{ 31, 26, 0x3C, true, RegClass::NotReg, "funct6" },
...
template <typename Ts2, typename Td> int run_self_result(c_data &cur_data)
{
	...
	for (int j = 0; j < vector_cfg.len; j++) {
		...
		// vd[i] = (unsigned(x[rs1]) * unsigned(vs2[i])) + vd[i]
		selfcheck_data[j] =
...
int main(int argc, char *argv[])
{
	...
	for (it = 0; it < run_times; it++) {
		...
		case sew_e8:
			...
			break;
		case sew_e16:
			has_error = per_run<uint16_t, uint32_t>(it, cur_cfg,
								cur_data);
			break;
		case sew_e32:
			has_error = per_run<uint32_t, uint64_t>(it, cur_cfg,
								cur_data);
			break;
		default:
			break;
