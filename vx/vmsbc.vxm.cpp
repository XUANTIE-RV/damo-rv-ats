...
template <typename Ts1, typename Ts2> int run_self_result(c_data &cur_data)
{
	...
	for (int j = 0; j < vector_cfg.len; j++) {
		...
		int borrow_in = (!vm_bit) ? (vm_data[j / 8] >> (j % 8)) & 1 : 0;
		...
		WideType subtrahend =
			static_cast<WideType>(static_cast<UTs1>(*rs1_data)) +
			borrow_in;
		int borrow_out = (minuend < subtrahend) ? 1 : 0;
...
int per_run(int it, c_cfg &cur_cfg, c_data &cur_data)
{
	...
	save_multi_preinst_value_to_common<Ts1, uint8_t, Ts2, uint8_t>(
