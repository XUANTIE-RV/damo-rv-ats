...
template <typename Ts2, typename Td> int run_self_result(c_data &cur_data)
{
	...
	// x[rs1] is XLEN bits, treated as unsigned, not truncated to SEW
	uint64_t *rs1_data = static_cast<uint64_t *>(
		cur_data.map_preinst_typed_value["rs1"].get());
	uint64_t OFFSET = *rs1_data;
	...
	for (int j = 0; j < vector_cfg.len; j++) {
		...
		// vslideup behavior:
		// 0 <= i < max(vstart, OFFSET): unchanged (already handled by vstart check above)
		// max(vstart, OFFSET) <= i < vl: vd[i] = vs2[i-OFFSET] if mask enabled
		if ((uint64_t)j < OFFSET) {
			// Element below OFFSET: unchanged
			continue;
		}
...
int per_run(int it, c_cfg &cur_cfg, c_data &cur_data)
{
	...
	save_multi_preinst_value_to_common<Td, Ts2, uint8_t, uint64_t>(
		...
		{ vector_cfg.len, vlmax_count, vlen, 1 });
