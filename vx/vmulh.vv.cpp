...
int run_self_result(c_data &cur_data)
{
	...
	for (int j = 0; j < vector_cfg.len; j++) {
		...
		if constexpr (sizeof(Ts1) == 8) {
			// SEW=e64: use __int128 for 64x64->128 multiplication
			__int128 wide_product =
				static_cast<__int128>(vs2_data[j]) *
				static_cast<__int128>(vs1_data[j]);
			selfcheck_data[j] = static_cast<Td>(wide_product >> 64);
		} else if constexpr (sizeof(Ts1) == 4) {
			int64_t wide_product =
				static_cast<int64_t>(vs2_data[j]) *
				static_cast<int64_t>(vs1_data[j]);
			selfcheck_data[j] = static_cast<Td>(wide_product >> 32);
		} else if constexpr (sizeof(Ts1) == 2) {
			int32_t wide_product =
				static_cast<int32_t>(vs2_data[j]) *
				static_cast<int32_t>(vs1_data[j]);
			selfcheck_data[j] = static_cast<Td>(wide_product >> 16);
		} else {
			int16_t wide_product =
				static_cast<int16_t>(vs2_data[j]) *
				static_cast<int16_t>(vs1_data[j]);
...
int main(int argc, char *argv[])
{
	...
	for (it = 0; it < run_times; it++) {
		...
		case sew_e8:
			has_error = per_run<int8_t, int8_t, int8_t>(it, cur_cfg,
								    cur_data);
			break;
		case sew_e16:
			has_error = per_run<int16_t, int16_t, int16_t>(
				it, cur_cfg, cur_data);
			break;
		case sew_e32:
			has_error = per_run<int32_t, int32_t, int32_t>(
				it, cur_cfg, cur_data);
			break;
		case sew_e64:
			has_error = per_run<int64_t, int64_t, int64_t>(
				it, cur_cfg, cur_data);
			break;
		default:
			break;
