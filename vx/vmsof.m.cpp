...
const std::vector<InstField> vop_inst_fields = {
	...
	{ 19, 15, 0x02, true, RegClass::NotReg,
	  "vs1" }, // vs1 fixed to 0x02 for vmsof.m
...
int run_self_result(c_data &cur_data)
{
	...
	// Write 1 only to the first active element that has vs2 bit set,
	// write 0 to all other active elements.
	// If no set bit in active elements, all active elements are written with 0.
	...
	for (int j = 0; j < vector_cfg.len; j++) {
		...
		if (!found_first && vs2_bit) {
			// Found the first set bit: write 1 to this element only
			selfcheck_data[byte_idx] |= (1u << bit_idx);
			found_first = true;
		} else {
			// All other active elements: write 0
			selfcheck_data[byte_idx] &= ~(1u << bit_idx);
		}
	}
...
int per_run(int it, c_cfg &cur_cfg, c_data &cur_data)
{
	...
	save_multi_preinst_value_to_common<uint8_t, uint8_t, uint8_t>(
		cur_data, { "vd", "vs2", "vm" }, { vlen, vlen, vlen });
